...
const errorColors: Record<string, { bg: string; border: string; text: string; underline: string }> = {
  grammar: { bg: 'bg-[#EF4444]/10', border: 'border-[#EF4444]/30', text: 'text-[#EF4444]', underline: 'error-underline' },
  spelling: { bg: 'bg-[#F59E0B]/10', border: 'border-[#F59E0B]/30', text: 'text-[#F59E0B]', underline: 'spelling-underline' },
  punctuation: { bg: 'bg-[#2563EB]/10', border: 'border-[#2563EB]/30', text: 'text-[#2563EB]', underline: 'error-underline' },
  syntax: { bg: 'bg-[#7C3AED]/10', border: 'border-[#7C3AED]/30', text: 'text-[#7C3AED]', underline: 'syntax-underline' },
};

export default function HighlightedSentence({ darkMode, text, errors, onApply }: HighlightedSentenceProps) {
  ...
  for (const error of sortedErrors) {
    ...
    segments.push({ text: text.slice(error.startIndex, error.endIndex), error });
    ...
    <section className="py-10">
      <div className="max-w-5xl mx-auto px-6">
        ...
        >
          ...
          >
            <p className={`text-lg leading-relaxed ${darkMode ? 'text-[#E2E8F0]' : 'text-[#334155]'}`}>
              {segments.map((seg, i) => {
                if (!seg.error) {
                  return <span key={i}>{seg.text}</span>;
                }

                const colors = errorColors[seg.error.type] || errorColors.grammar;
                        ...
                        >
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className={`text-xs font-bold uppercase tracking-wider ${colors.text}`}>
                              {seg.error!.type} Error
                            </span>
                          </div>
                          <p className={`text-sm font-medium mb-1 ${darkMode ? 'text-[#94A3B8]' : 'text-[#64748B]'}`}>
                            {seg.error!.rule}
                          </p>
                          <div className="flex items-center gap-2 mt-3 mb-3">
                            <span className={`text-sm line-through ${darkMode ? 'text-[#64748B]' : 'text-[#94A3B8]'}`}>
                              {seg.error!.original}
                            </span>
                            <span className={`text-sm ${darkMode ? 'text-[#64748B]' : 'text-[#94A3B8]'}`}>→</span>
            ...
            {/* Legend */}
            <div className={`flex items-center gap-5 mt-6 pt-5 border-t flex-wrap ${
              ...
              {Object.entries(errorColors).map(([type, colors]) => (
                <div key={type} className="flex items-center gap-2">
                  ...
                  <span className={`text-xs font-medium capitalize ${darkMode ? 'text-[#64748B]' : 'text-[#94A3B8]'}`}>
                    {type}
                  </span>
                </div>
              ))}
